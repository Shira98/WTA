...
class Di_Graph:
	...
	def Tree_Set_Creation(self):
		...
		for i in range(len(self.doc_list)):
			...
			self.docs_objects.append(Document(self.doc_list[i]))
	...
	def Content_Score(self):
		...
		for element in self.retrieved:
			print(element.score,element.element)
			element.content_score=self.idf*element.score
			#print(element.content_score)
			self.docs_objects[element.doc].content_score+=element.content_score
	...
	def link_score_computation(self,damp_factor=0.85): 
		...
		for no in range(N): 
			...
			self.docs_objects[self.retrieved[no].doc].link_score+=link_sum


	def Normalization(self): #LS and Content score.
		for i in self.docs_objects:
			
			index=self.docs_objects.index(i)
			i.relevance_score=fuzzy(i.content_score,i.link_score)

			print('scores',i.content_score,i.link_score,i.relevance_score)

			self.docs_objects[index].relevance_score=i.relevance_score
			#Return Relevant score and calculate for a whole doc.

	def arrange(self):

		#Sorting the documents based on content score:

		content_sort=sorted(self.docs_objects, key=lambda x: x.content_score, reverse=True)

		relevant_sort=sorted(self.docs_objects, key=lambda x: x.relevance_score, reverse=True)
		print("Query results based on initial content scores: ")
		for doc in content_sort:
			print(doc.name,doc.content_score)
		
		print("Query results based on final relevant scores: ")

		for doc in relevant_sort:
			print(doc.name,doc.relevance_score)
